...
class ContadorPedidos:
    ...
    def calcular_distancia(self, arquivo_pedido, arquivo_distancia, arquivo_saida):
        ...
        logging.info(
            f"Tempo de execução do Calculo {round(end_time-init_time,2)} segundos "
        )
        return df
    ...
    def calcular_resultados(self, arquivo_df1, arquivo_df2):
        ...
        # criando um dicionário com os valores de COD inicial como chave e a capacidade de entrega e o nome HUB correspondentes como valor
        cod_inicial_dict = {
            row["COD inicial"]: (row["Capacidade Entrega"], row["Nome HUB"])
            for _, row in df2.iterrows()
        }

        # função para buscar a capacidade de entrega correspondente ao COD inicial
        def get_capacidade_entrega(cod_org_x):
            cod_inicial_prox = min(
                cod_inicial_dict.keys(), key=lambda x: abs(x - cod_org_x)
            )
            return cod_inicial_dict.get(cod_inicial_prox, (None, None))

        # aplicando a função get_capacidade_entrega em todas as linhas de df1 e salvando o resultado na coluna Resultado
        df1["Capacidade Entrega"], df1["Nome HUB"] = zip(
            *df1["COD org_x"].apply(get_capacidade_entrega)
        )
        df1["Resultado"] = (
            round(df1["Capacidade Entrega"] / df1["Distancia total"], 2) * 2
        )
    ...
    def excluir_file(self, lista_arquivos):
        ...
        df3 = pd.read_csv(
            "dados/PEDIDOS_HUBS_DISTANCIA_DIAS.csv", sep=",", encoding="cp1252"
        )

        # Excluir as colunas desejadas
        df3 = df3.drop(["COD org_y", "COD_dest_y"], axis=1)

        # Salvar o arquivo sem as colunas excluídas
        df3.to_csv(
        ...
        # escreve os dataframes em abas separadas no arquivo Excel, dividindo os dados em planilhas menores
        size_limit = 1048576
        n_rows = len(df1)
        for i in range(0, n_rows, size_limit):
            df1.iloc[i : i + size_limit].to_excel(
                writer, sheet_name=f"aba1_{i//size_limit+1}", index=False
            )

        n_rows = len(df2)
        for i in range(0, n_rows, size_limit):
            df2.iloc[i : i + size_limit].to_excel(
                writer, sheet_name=f"aba2_{i//size_limit+1}", index=False
            )
